...
def register (request):
      if request.method =="POST":
            ...
            mycustomer=Customer(user = user, first_name = fname, last_name = lname, email=email, password=password, phone_number=phonenumber, gender=gender, role = classify)                                
            mycustomer.save()
...
def custom_login(request):
      if request.method == "POST":
            ...
            if user is not None:
                  ...
                  return render(request, "quiz/landing.html", {'username': username})
